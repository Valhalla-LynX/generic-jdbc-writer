    ...
    public void run() {
        ...
        while (state.get()) {
            synchronized (messageList) {
                boolean overload = messageList.size() >= ConfigFileReaderConf.getList();
                if ((overload || System.currentTimeMillis() - lastProcessTime >= ConfigFileReaderConf.getCycle()) && !messageParseWriteRunnable.getWriting().get()) {
                    if (!messageList.isEmpty()) {
                        topicTableMapperProcessor.addMessageCount((long) messageList.size());
